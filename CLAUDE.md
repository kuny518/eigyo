...
stateDiagram-v2
    ...
    下書き --> 提出済: 提出
    提出済 --> 確認済: 上長が確認
    提出済 --> 差し戻し: 上長が差し戻し(理由必須)
    差し戻し --> 提出済: 再提出
```

| ステータス | 値 | 本人の編集 | 説明 |
| --- | --- | :-: | --- |
| 下書き | DRAFT | 可 | 作成直後。未提出 |
| 提出済 | SUBMITTED | 不可 | 上長の確認待ち |
| 差し戻し | RETURNED | 可 | 上長が理由を付けて差し戻した。本人が修正して再提出する |
| 確認済 | REVIEWED | 不可 | 上長が確認した。以降は変更しない |

コメントの投稿ではステータスは変わらない(確認は「確認済にする」操作で行う)。
...
erDiagram
    ...
    DAILY_REPORT {
        ...
        text problem "課題・相談"
        text plan "明日やること"
        string status "DRAFT/SUBMITTED/RETURNED/REVIEWED"
...
- [x] ~~提出後の編集可否と、差し戻し運用の有無~~ → 提出済・確認済は編集不可。差し戻しは独立ステータス(RETURNED)とし、本人が修正・再提出する(#2)
