...
| SCR-05 | 日報詳細・コメント | 日報の閲覧、Problem/Planへのコメント、確認・差し戻し | ○ | ○ | ○ | F-06, F-07 |
...
| No | 項目名 | 種別 | 表示ロール | 備考 |
| --- | --- | --- | --- | --- |
| 1 | 今日の日報ステータス | ラベル | 営業 | 未作成／下書き／提出済／確認済 |
| 2 | 今日の日報を書く | ボタン | 営業 | 未作成なら新規、作成済なら編集でSCR-04へ |
| 3 | 下書き・差し戻し一覧 | リスト | 営業 | 報告日、ステータス。クリックでSCR-04 |
| 4 | 新着コメント | リスト | 営業 | 日付、コメント者、対象（Problem/Plan）、本文冒頭30文字 |
...
| --- | --- | --- |
| キャンセル | 未保存確認後に戻る | 前画面 |
| 下書き保存 | status=DRAFTで保存。必須チェックは顧客のみ | 同画面（トースト表示） |
...
**目的**：日報内容を確認し、上長がProblem/Planそれぞれにコメントする。
...
| 差し戻す | 上長かつ提出済 | 差し戻し理由（コメント）必須→status=DRAFT、営業に通知 |
...
- [ ] 営業がコメントに返信できるようにするか（SCR-05のスレッド化）
- [ ] 差し戻しをステータスとして独立させるか（現案はDRAFTに戻す。ER図のstatusに「RETURNED」を追加する案もあり）
- [ ] 業種の選択肢の定義
